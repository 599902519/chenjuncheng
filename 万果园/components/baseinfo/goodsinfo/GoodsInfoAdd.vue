...
<script>
	export default {
		...
		methods:{
			saveAdd(){
				// 后端网址
				var url = this.baseUrl1+"/goodsInfo/insert"
				// 传递给后端的数据
				var data = {gname:this.gname,marketprice:this.marketprice,shopprice:this.shopprice,image:this.image,gdesc:this.gdesc,gdate:this.gdate,cid:this.cid,stocknumber:this.stocknumber,salenumber:this.salenumber,shopid:this.shopid};
